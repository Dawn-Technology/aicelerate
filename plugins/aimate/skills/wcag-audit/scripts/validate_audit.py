...
INVENTORY_SIGNAL_ORDER = (
    ...
    "auth_entry_points",
    "help_contact",
    "uninvoked_resize_aria_handlers",
    "responsive_aria_initial_mismatches",
    "definite_low_text_contrast",
    "undersized_interactive_styles",
    "skip_links",
    "main_content_targets",
    "targetless_mains",
    "listbox_roles",
    "option_roles",
    "unannounced_dynamic_updates",
)
...
def focus_suppression_count(target: Path, scopes: list[Path] | None = None) -> int:
    ...
    return len(
        source_signal_locations(
            target, {".css", ".scss", ".sass", ".less"}, pattern, scopes
        )
    )
...
def suspicious_focus_suppressions(target: Path, scopes: list[Path] | None = None) -> list[str]:
    """Find focus blocks that remove outline without a local presentation change."""
    ...
    replacement = re.compile(
        r"\b(?:box-shadow|border(?:-color)?|background(?:-color)?|color|opacity|filter|transform|text-decoration)\s*:"
...
def render_inventory_markdown(inventory: dict[str, object]) -> str:
    """Render the machine-owned inventory count contract for report inclusion."""
    counts = inventory["counts"]
    roots = inventory["scopes"]
    lines = [
        "## Deterministic source inventory",
        "",
        "<!-- Generated by validate_audit.py; insert unchanged and do not edit counts manually. -->",
        f"**Inventory target:** `{inventory['target']}`",
        "",
        "**Inventory roots:** " + "; ".join(f"`{root}`" for root in roots),
        "",
        "| Signal | Raw count |",
        "|---|---:|",
    ]
    lines.extend(f"| `{name}` | {counts[name]} |" for name in INVENTORY_SIGNAL_ORDER)
    return "\n".join(lines)


def validate_embedded_inventory(
    text: str,
    target: Path | None,
    scopes: list[Path] | None,
) -> list[str]:
    """Compare the report's generated inventory table with the audited source."""
    if target is None or not target.is_dir() or any(not scope.is_dir() for scope in scopes or []):
        return []
    section = re.search(
        r"^## Deterministic source inventory\s*$([\s\S]*?)(?=^## |\Z)",
        text,
        re.MULTILINE,
    )
    if not section:
        return ["report is missing the machine-generated Deterministic source inventory section"]
    ...
    expected = build_source_inventory(target, scopes)["counts"]
    errors: list[str] = []
    if duplicates:
        errors.append(
            "deterministic source inventory contains duplicate signals: "
            + ", ".join(sorted(duplicates))
        )
    missing = [name for name in INVENTORY_SIGNAL_ORDER if name not in actual]
    extra = sorted(set(actual) - set(INVENTORY_SIGNAL_ORDER))
    if missing:
        errors.append("deterministic source inventory is missing signals: " + ", ".join(missing))
    if extra:
        errors.append("deterministic source inventory contains unknown signals: " + ", ".join(extra))
    for name in INVENTORY_SIGNAL_ORDER:
        if name in actual and actual[name] != expected[name]:
            errors.append(
                f"deterministic source inventory mismatch for {name}: "
                f"report={actual[name]}, authored source={expected[name]}"
            )
    return errors
...
def validate_partial_report(
    ...
    errors.extend(validate_embedded_inventory(text, target, scopes))
...
def validate_report(
    ...
    focus_visible_claim = (
        rows_by_id.get("2.4.7", {}).get("evidence", "") + "\n" + detail_for("2.4.7")
    )
    if rows_by_id.get("2.4.7", {}).get("verdict") == "❌ FAIL" and re.search(
        r"(?:identical|same|shared)[\s\S]{0,180}(?:hover[\s\S]{0,80}focus|focus[\s\S]{0,80}hover)"
        r"[\s\S]{0,220}(?:not distinguishable|no distinct (?:focus )?indicator)",
        focus_visible_claim,
        re.IGNORECASE,
    ):
        errors.append(
            "2.4.7 incorrectly treats a focus style shared with hover/active as a failure; "
            "compare the focused presentation with the unfocused state instead"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--inventory",
        action="store_true",
        help="print deterministic source inventory before evaluation (requires --target)",
    )
    parser.add_argument(
        "--inventory-format",
        choices=("json", "markdown"),
        default="json",
        help="inventory output format; markdown is the exact report section (default: json)",
    )
    parser.add_argument(
        "--scope",
        action="append",
    ...
    if errors:
        ...
        return 1
    if args.inventory:
        inventory = build_source_inventory(args.target, scopes)
        if args.inventory_format == "markdown":
            print(render_inventory_markdown(inventory))
        else:
